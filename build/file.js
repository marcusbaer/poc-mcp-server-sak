...
export class FileTools extends BaseTools {
    ...
    getTools() {
        return [
            {
                name: "read-file",
                description: "Read a file from user home directory",
            ...
            {
                name: "write-file",
                description: "Write a file to user home directory",
                inputSchema: z.object({
                    filename: z.string().describe("Name of a file"),
    ...
    async writeFileHandle({ filename = "", body = "" }) {
        const dir = path.join(homedir(), filename);
